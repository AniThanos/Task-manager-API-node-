...
const {sendWelcomeMessage,sendCancellationMessage} =require('../emails/accounts')
...
//upload profile pic
const upload = multer({
    //dest : 'avatars', storing image data as buffer,one must comment this, we ll get access to req.file.buffer
    limits : {
        fileSize : 1000000
    },
    fileFilter(req,file,cb) {
        if(!file.originalname.match(/\.(jpg|jpeg|png)$/)){
            return cb(new Error('jpg,jpeg,png allowed'))
        }
        cb(undefined, true)
    }
})
